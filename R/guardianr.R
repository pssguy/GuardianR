...
get_json <- function(keywords, format="json", from.date, to.date, fields=c("headline","byline"), api.key)
{
  # pagination
  page.size <- 50
  this.page <- 1
  ...
  {
    request <- paste("http://beta.content.guardianapis.com/search?q=", keywords, "&from-date=", from.date, "&to-date=", to.date, "&format=", format, "&show-fields=", fields, "&page=", this.page, "&pageSize=", page.size, "&api-key=", api.key, sep="")
